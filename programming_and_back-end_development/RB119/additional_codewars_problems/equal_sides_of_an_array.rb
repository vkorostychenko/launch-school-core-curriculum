...
Input: an array
Output: -1 or an index

Rules:
  - given an array of integers
  - return the index N where the sum of the integers on the left is equal to the
    sum of integers on the right
...
Algorithm:
  ...
  - iterate over each index of the input array
    - slice the array from infinity up to the current index exclusively and
      assign the result to a local variable left
    - slice the array from the current index + 1 up to infinity
    - return the current index if the sum of left is equal to the sum of right
  - return -1
...
def find_even_index(arr)
  arr.each_index do |idx|
    return idx if arr[...idx].sum == arr[idx + 1..].sum
  end
