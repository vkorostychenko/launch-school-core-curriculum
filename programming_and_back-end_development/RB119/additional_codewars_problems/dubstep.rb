...
music in his performances. Recently, he has decided to take a couple of old songs 
...
pair of neighboring words), and then the boy glues together all the words, 
...
Rules:
  - given a non-empty string consisting of only uppercase English letters
  - return a new string with all instances of "WUB" removed
    - words must be separated by single spaces

Algorithm:
  - given a string
  - split the string into an array of words between "WUB"
  - reject empty strings from the array
  - join the array with ' ' and return the result string
=end
